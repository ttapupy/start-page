...
const skeletonCardClass = twMerge(
  "bg-papirus_light dark:bg-crt_background rounded-lg border-2 border-gray-300 dark:border-gray-700 shadow-sm max-w-[420px] p-4 flex flex-col justify-between animate-pulse",
);

export default function FeedCardSkeleton({ count = 5 }: { count?: number }) {
  return (
    <div className="px-auto mr-auto min-w-[320px] max-w-[100%] flex-[0_0_auto] pb-[1em]">
      {/* Skeleton cards */}
      {Array.from({ length: count }).map((_, idx) => (
        <div key={idx} className={skeletonCardClass} data-testid="skeleton-card">
